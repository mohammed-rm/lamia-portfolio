...
const ProjectCard = ({ title, keywords, link }) => {
  return (
    <>
      <Card
        radius="sm"
        className="w-[400px] max-w-[400px] h-auto bg-[conic-gradient(at_top_right,_var(--tw-gradient-stops))] from-white via-blue-100 to-pink-100 shadow-md border-1 border-primary"
      >
        ...
        <CardFooter className="flex justify-end h-auto p-4">
          <Link href={link} target={"_blank"}>
            <Button
              className="rounded-[4px] text-primary sm:tracking-wide text-sm md:text-medium font-playpen"
              color="primary"
              variant="bordered"
            >
              Code source
